...
Notes:
- Set `CMAKE_CUDA_ARCHITECTURES` to match your GPU (for example, `80`, `86`, `89`, or `90`).
- The resulting static library is `gpu/build/libsimplepir_cutlass.a`.

### Use CUTLASS from Go

The Go package keeps CPU as default. To enable GPU matmul:

```
export SIMPLEPIR_USE_CUTLASS=1
go test -tags cutlass ./pir
```
